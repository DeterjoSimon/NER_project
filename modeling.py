...
class BertForTokenClassification_(BertForTokenClassification):
    ...
    def forward_wuqh(
        ...
        max_len = (attention_mask != 0).max(
            0)[0].nonzero(as_tuple=False)[-1].item() + 1
        ...
        if e_type_ids is not None and self.train_mode != "span":
            ...
            if is_update_type_embedding:
                entity_types.update_type_embedding(
            ...
            if attention_mask is not None:
                ...
                if lambda_max_loss > 0:
                    ...
                    for i in range(B):
                        ...
                        active_max.append(
                            torch.max(base_loss[start_id:end_id]))
                        start_id = end_id

                    loss += lambda_max_loss * \
                        torch.mean(torch.stack(active_max))
            else:
    ...
    def forward(self, logprobs, attention_mask, label_ids):
        ...
        for idx in range(batch_size):
            ...
            for j in range(1, len(logprob_i)):  # for tag_feat in feat:
                ...
                viterbivars_t, bptrs_t = torch.max(
                    next_label_var, dim=1)  # n_labels
