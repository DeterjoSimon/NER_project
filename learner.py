...
class Learner(nn.Module):
    ...
    def __init__(
        ...
        if model_dir != "":
            ...
            cache_dir = cache_dir if cache_dir else str(
    ...
    def layer_set(self):
        ...
        self.opt = BertAdam(
            self.get_optimizer_grouped_parameters(), lr=self.lr_meta)
    ...
    def inner_update(self, data_support, lr_curr, inner_steps, no_grad: bool = False):
        inner_opt = BertAdam(
            self.get_optimizer_grouped_parameters(), lr=self.lr_inner)
        self.model.train()
    ...
    def forward_meta(self, batch_query, batch_support, progress, inner_steps):
        ...
        for task_id in range(task_num):
            self.inner_update(
                batch_support[task_id], lr_inner, inner_steps=inner_steps)
            _, _, loss, type_loss = self.model.forward_wuqh(
                input_ids=batch_query[task_id]["input_ids"],
                attention_mask=batch_query[task_id]["input_mask"],
                token_type_ids=batch_query[task_id]["segment_ids"],
    ...
    def batch_test(self, data):
        ...
        for i in range((N - 1) // B + 1):
            ...
            tmp_l, tmp_el, tmp_loss, tmp_eval_type_loss = self.model.forward_wuqh(
                **tmp)
    ...
    def evaluate_meta_(
        ...
        if not type_steps:
            type_steps = steps
        if self.is_debug:
            self.save_model(self.args.result_dir, "begin",
                            self.args.max_seq_len, "all")
        ...
        for item_id in range(corpus.n_total):
            ...
            if not self.args.nouse_inner_ft:
                self.inner_update(
                    eval_support[0], lr_curr=lr, inner_steps=steps)
        ...
        if self.train_mode == "add":
            for item_id in range(corpus.n_total):
                ...
                # train on support examples
                self.inner_update(
                    eval_support[0], lr_curr=lr, inner_steps=type_steps)
    ...
    def save_model(self, result_dir, fn_prefix, max_seq_len, mode: str = "all"):
        ...
        if mode == "type":
            joblib.dump(
                self.entity_types, os.path.join(
                    result_dir, "type_embedding.pkl")
            )

    def save_best_model(self, result_dir: str, mode: str):
        output_model_file = os.path.join(
            result_dir, "en_tmp_{}".format(WEIGHTS_NAME))
    ...
    def load_model(self, mode: str = "all"):
        ...
        self.model.load_state_dict(torch.load(
            output_model_file, map_location="cuda"))
        self.layer_set()
    ...
    def decode_span(
        ...
        if viterbi_decoder:
            ...
            result = []
            for i in range((N - 1) // B + 1):
                tmp_logits = torch.tensor(
                    logits[i * B: (i + 1) * B]).to(target.device)
                ...
                for ii, jj in zip(pred_labels, tmp_target.detach().cpu().numpy()):
                    ...
                    while left < len(ii):
                        ...
                        while (
                            right < len(
                                jj) and jj[right] == self.ignore_token_label_id
                        ):
                            tmp.append(-1)
                            right += 1
                    result.append(tmp)
        ...
        if self.label_list == ["O", "B", "I"]:
            ...
            for ii in range(B):
                ...
                max_pad = T - 1
                while (
                    max_pad > 0 and target[ii][max_pad -
                                               1] == self.pad_token_label_id
                ):
                    max_pad -= 1
                while idx < max_pad:
    ...
    def decode_entity(self, e_logits, result, types, entities):
        ...
        for ii in range(B):
            ...
            for jj in range(len(tmp_res)):
                ...
                tmp.append(
                    (*tmp_res[jj], tmp_types[np.argmax(lg)], lg[np.argmax(lg)]))
            preds.append(tmp)
        return target, preds
